...
svg_js.extend(svg_js.PathArray, {
  ...
  toPoly(sample = '1%') {
    ...
    const segmentsQueue = this.slice(); // prepare helpers functions

    const addPoint = function (px, py) {
      // get last point
      const lastPoint = points[points.length - 1]; // when the last point doesn't equal the current point add the current point
    ...
    let subPathLength = pathLength(subPath); // sample through path
    ...
    while (length < total) {
      ...
      const segment = this[segmentIndex]; // new segment?
    ...
    let i = 0;
    let il = segmentsQueue.length; // add remaining segments we didn't pass while sampling
...
svg_js.extend(svg_js.Path, {
  ...
  toPoly(sample = '1%', replace = true) {
    // define type
    const Poly = /z\s*$/i.test(this.attr('d')) ? svg_js.Polygon : svg_js.Polyline;
    const pointArray = this.array().toPoly(sample); // create poly

    const poly = new Poly().plot(pointArray).attr(normaliseAttributes(this.attr())); // insert poly
...
//# sourceMappingURL=svg.topoly.node.js.map
