...
svg_js.extend(svg_js.Shape, {
  ...
  toPath(replace = true) {
      ...
      case 'rect':
        {
          let {
            ...
            x,
            y
          } = this.attr(['width', 'height', 'rx', 'ry', 'x', 'y']); // normalise radius values, just like the original does it (or should do)
    ...
    const path = new svg_js.Path().plot(d).attr(normaliseAttributes(this.attr()));
